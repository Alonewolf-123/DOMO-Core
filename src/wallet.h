...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 DOMO at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zDOMO send
enum ZerocoinSpendStatus {
    ZDOMO_SPEND_OKAY = 0,                            // No error
    ZDOMO_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZDOMO_WALLET_LOCKED = 2,                         // Wallet was locked
    ZDOMO_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZDOMO_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZDOMO_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZDOMO_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZDOMO_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZDOMO_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZDOMO_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZDOMO_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZDOMO_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZDOMO_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZDOMO_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZDOMO_SPENT_USED_ZDOMO = 14,                      // Coin has already been spend
    ZDOMO_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZDOMO_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZDomoBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZDOMOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzDOMOWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzDOMOTracker> zpivTracker;
    ...
    void setZWallet(CzDOMOWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzDOMOTracker>(new CzDOMOTracker(strWalletFile));
    }

    CzDOMOWallet* getZWallet() { return zwalletMain; }
    ...
    void setZDomoAutoBackups(bool fEnabled)
    {
    ...
    /** zDOMO reset */
    boost::signals2::signal<void()> NotifyzDOMOReset;
