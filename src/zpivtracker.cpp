...
CzDOMOTracker::CzDOMOTracker(std::string strWalletFile)
{
...
CzDOMOTracker::~CzDOMOTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzDOMOTracker::Init()
{
...
bool CzDOMOTracker::Archive(CMintMeta& meta)
{
...
bool CzDOMOTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzDOMOTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzDOMOTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzDOMOTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzDOMOTracker::GetSerialHashes()
{
...
CAmount CzDOMOTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzDOMOTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzDOMOTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzDOMOTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzDOMOTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzDOMOTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzDOMOTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzDOMOTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzDOMOTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzDOMOTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzDOMOTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDOMOTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDOMOTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzDOMOTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzDOMOTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzDOMOTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzDOMOTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
