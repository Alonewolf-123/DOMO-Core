...
Value getgenerate(const Array& params, bool fHelp)
{
	...
	throw runtime_error(
			...
			"It is set with the command line argument -gen (or Domocoin.conf setting gen)\n"
...
Value prioritisetransaction(const Array& params, bool fHelp) {
	if (fHelp || params.size() != 3)
		throw runtime_error(
				"prioritisetransaction <txid> <priority delta> <fee delta>\n"
						...
						"\nExamples:\n"
						+ HelpExampleCli("prioritisetransaction",
								"\"txid\" 0.0 10000")
	...
	mempool.PrioritiseTransaction(hash, params[0].get_str(),
			params[1].get_real(), nAmount);
	return true;
}
...
Value getblocktemplate(const Array& params, bool fHelp) {
	if (fHelp || params.size() > 1)
		throw runtime_error(
				"getblocktemplate ( \"jsonrequestobject\" )\n"
						...
						"             ,...\n"
						"         ],\n"
						"         \"fee\": n,                   (numeric) difference in value between transaction inputs and outputs (in Domos); for coinbase transactions, this is a negative Number of the total collected block fees (ie, not including the block subsidy); if key is not present, fee is unknown and clients MUST NOT assume there isn't one\n"
						...
						"      \"flags\" : \"flags\"            (string) \n"
						"  },\n"
						"  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in Domos)\n"
	...
	if (vNodes.empty())
		throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED,
				"Domocoin is not connected!");

	if (IsInitialBlockDownload())
		throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD,
				"Domocoin is downloading blocks...");
	...
	if (lpval.type() != null_type) {
		...
		{
			checktxtime = boost::get_system_time()
	...
	aux.push_back(
			Pair("flags",
					HexStr(COINBASE_FLAGS.begin(), COINBASE_FLAGS.end())));
	...
	if (pblock->payee != CScript()) {
		...
		result.push_back(
				Pair("payee_amount", (int64_t) pblock->vtx[0].vout[1].nValue));
	} else {
	...
	result.push_back(
			Pair("masternode_payments",
					pblock->nTime > Params().FirstMasternodePaymentBlock()));
