...
#ifndef DOMO_ZDOMOWALLET_H
#define DOMO_ZDOMOWALLET_H
...
public:
    CzDOMOWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZDOMO(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZDOMO(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //DOMO_ZDOMOWALLET_H
