...
CzDOMOWallet::CzDOMOWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDOMOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDOMOSeed_deprecated()) {
                    LogPrintf("%s: Updated zDOMO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzDOMOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDOMOCount(nCountLastUsed);
    else if (!walletdb.ReadZDOMOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDOMOWallet::Lock()
{
    seedMaster = 0;
}

void CzDOMOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDOMOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDOMO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDOMOWallet::LoadMintPoolFromDB()
{
...
void CzDOMOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDOMOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDOMOWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzDOMOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDOMO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzDOMOWallet::SeedToZDOMO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDOMOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDOMOWallet::GenerateDeterministicZDOMO(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDOMOWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDOMOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
