...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::DOMO)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sDOMOPercentage, QString& szDOMOPercentage)
{
    int nPrecision = 2;
    ...
    szDOMOPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sDOMOPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // Percentage labels
    ui->labelDOMOPercent->setText(sPercentage);
    ui->labelzDOMOPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zDOMO.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in domo.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in domo.conf");
    }
    ...
    bool showDOMOAvailable = settingShowAllBalances || pivAvailableBalance != nTotalBalance;
    bool showWatchOnlyDOMOAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showDOMOPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyDOMOPending = watchUnconfBalance != 0;
    bool showDOMOLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyDOMOLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showDOMOAvailable || showWatchOnlyDOMOAvailable);
    ui->labelBalanceText->setVisible(showDOMOAvailable || showWatchOnlyDOMOAvailable);
    ui->labelWatchAvailable->setVisible(showDOMOAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showDOMOPending || showWatchOnlyDOMOPending);
    ui->labelPendingText->setVisible(showDOMOPending || showWatchOnlyDOMOPending);
    ui->labelWatchPending->setVisible(showDOMOPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showDOMOLocked || showWatchOnlyDOMOLocked);
    ui->labelLockedBalanceText->setVisible(showDOMOLocked || showWatchOnlyDOMOLocked);
    ui->labelWatchLocked->setVisible(showDOMOLocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("DOMO")
    updateDisplayUnit();
}
