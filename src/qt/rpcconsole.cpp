...
const struct {
    const char* url;
    const char* source;
} ICON_MAPPING[] = {{"cmd-request", ":/icons/tx_input"}, {"cmd-reply", ":/icons/tx_output"}, {"cmd-error", ":/icons/tx_output"}, {"misc", ":/icons/tx_inout"}, {NULL, NULL}};

RPCConsole* RPCConsole::rpc_instance;
...
RPCConsole::RPCConsole(QWidget* parent) : QDialog(parent), ui(new Ui::RPCConsole), clientModel(0), historyPtr(0), cachedNodeid(
                                                                                                                      -1)
{
    ui->setupUi(this);
    ...
    rpc_instance = this;
    std::signal(SIGINT, RPCConsole::static_SigHandler);
    clear();
}
...
void RPCConsole::SigHandler(int signum)
{
    QStringList args;
    args.append("-txindex=1");
    args.append("-reindex=1");
...
void RPCConsole::clear()
{
    ...
    message(CMD_REPLY,
            (tr("Welcome to the Domocoin RPC console.") + "<br>" + tr("Use up and down arrows to navigate history, and <b>Ctrl-L</b> to clear screen.") + "<br>" + tr("Type <b>help</b> for an overview of available commands.")),
            true);
}
...
void RPCConsole::updateNodeDetail(const CNodeCombinedStats* stats)
{
    ...
    if (!stats->nodeStats.addrLocal.empty())
        peerAddrDetails += "<br />" + tr("via %1").arg(
