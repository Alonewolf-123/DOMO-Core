...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake() && wtx.GetDepthInMainChain() >= 0) // ppcoin: coinstake transaction
    {
        TransactionRecord sub(hash, nTime);
        CTxDestination address;

        if (wtx.GetValueOut() - nDebit > 0) {
            if (isminetype mine = wallet->IsMine(wtx.vout[1])) {
                if (nNet > 0) {
                    ExtractDestination(wtx.vout[1].scriptPubKey, address);
                    // BAC stake reward
                ...
                if (wallet->IsMine(wtx.vout[nIndexMN])) {
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine && wtx.GetDepthInMainChain() >= 0) {
                TransactionRecord sub(hash, nTime);
                CTxDestination address;
