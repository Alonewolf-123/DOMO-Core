...
nSubsidy = 1 * COIN;
} else if (nHeight >= Params().FirstMasternodePaymentBlock()
	&& nHeight < 6000) {
nSubsidy = 4 * COIN;
...
bool FindBlockPos(CValidationState &state, CDiskBlockPos &pos,
	unsigned int nAddSize, unsigned int nHeight, uint64_t nTime, bool fKnown =
			false) {
LOCK(cs_LastBlockFile);
...
string strStop;
string strStart;
strStop = strPath + "/Domocoin-cli stop";
strStart = strPath + "/Domocoind";
...
void static CheckBlockIndex() {
	...
	while (pindex != NULL) {
		...
		assert(
				pindex->pprev == NULL
						|| pindex->nChainWork >= pindex->pprev->nChainWork); // For every block except the genesis block, the chainwork must be larger than the parent's.
		assert(
				nHeight < 2
...
if (strCommand == "tx") {
	...
	if (pmn != NULL) {
		...
		std::string strMessage = tx.GetHash().ToString() + boost::lexical_cast
				< std::string > (sigTime);
...
* (4 + state.vBlocksInFlight.front().nValidatedQueuedBefore)) {
